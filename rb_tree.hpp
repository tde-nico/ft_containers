...
namespace ft
{
	...
	struct Node 
	{
		...
		bool is_end;
		T pair;
		Node(): left(NULL), right(NULL), parent(NULL), is_black(false), is_end(false) {}
		Node(T value): left(NULL), right(NULL), parent(NULL), is_black(false), is_end(false), pair(value) {}
		~Node(){}
	};

	// Tree

	template < class _Tp, class Iterator, class Const, class _Compare, class Allocator = std::allocator<Node<_Tp> > >
	class __tree
	{
	public:
		...
		nodeptr get_end_node() const
		{
			return _end_node;
		}

		nodeptr get_begin_node() const
		{
			return _begin_node;
		}
		...
		const Allocator& get_alloc() const
		{
			return _alloc;
		}
